			...
			<div class="modal-dialog modal-dialog-centered" role="document">
				<div class="modal-content modal-content-demo">
					...
					<div class="modal-body" style='background: #dfe1e4'>
						...
						<br>
						{{Form::open(['method' => 'PUT' , 'url' => 'admin/products'])}}
						...
						<select class='form-control' name='section_id' value=''>
							<option class='d-none'  value=''>{{trans('admin.pleaseSelect')}}</option>
							@foreach($sections as $row)
							<option value='{{$row->id}}'>{{$row->name}}</option>
							@endforeach
			...
			<div class="modal-dialog modal-dialog-centered" role="document">
				<div class="modal-content modal-content-demo">
					...
					<div class="modal-footer">
						...
						{{Form::open(['method' => 'delete' , 'url' => 'admin/products/destroy'])}}
    ...
    @endif
							...
							<div class="card-body">
								<div class="table-responsive">
										{{Form::open(['id' =>'del' ,'method' => 'delete' , 'url' => 'admin/products/destroy'])}}
										...
										<tbody>
											...
											@foreach($data as $row)
												...
												<td>{{$row->name}}</td>
												 ...
												 <td>{{App\sections::where('id' , $row->section_id )->first()->name}}</td> 
												
												<td>{{$row->addBy}}</td>
												<td>{{$row->created_at}}</td>
												@can('product-edit')
												<td><a href="#" class='btn btn-success edit_section' id='{{$row->id}}' data-name='{{$row->name}}' data-discrption='{{$row->section_id}}'><i class='fas fa-edit '></i></a></td>
												@endcan
												@can('product-delete')
...
<script>
	...
	$('.edit_section').click(function(){
		...
		$('#modaldemo9 form').attr('action' , '{{url("admin/products")}}/'+id);
